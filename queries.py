...
class Queries:
    ...
    def query_7(self, activity, trackpoint):
        """
    ...
    def query_8(self, trackpoint):
        ...
        pprint(list(trackpoint.aggregate([
            ...
            # Make a window in which each trackpoint gets combined with the altitude of the next trackpoint
            # (based on id) if they belong to the same activity
            {
                "$setWindowFields": {
            ...
            # Calculate difference in altitude with its next trackpoint
            {
                "$project": {
                    "_id": 1,
                    "activity_id": 1,
                    "altitude_diff": {"$subtract": ["$next_altitude", "$altitude"]},
                }
            },
            # Only deal with positive altitude difference (as mentioned in tip of assignment)
            {"$match": {"altitude_diff": {"$gt": 0}}},
            # Sum the altitude differences to get the altitude gained per activity
            {
                "$group": {
                    "_id": "$activity_id",
                    "altitude_gained_per_activity": {"$sum": "$altitude_diff"},
                }
            },
            ...
            # Sum the altitude differences to get the altitude gained per user
            {
                "$group": {
                    "_id": "$JoinUser._id",
                    "altitude_gained": {"$sum": "$altitude_gained_per_activity"},
                }
            },
            # Sort the altitudes descending (in order to get the 20 biggest altitude gains)
            {"$sort": {"altitude_gained": -1}},
            # Select 20 user with the highest altitude gains
            {"$limit": 20},
            # Convert to altitiude gain to meters
            {
                "$project": {
                    "_id": "$_id",
                    "altitude_gained": {"$multiply": ["$altitude_gained", 0.3048]},
                }
            }
        ])))
...
def main():
    ...
    try:
        ...
        print('Query 6b: ')
        program.query_6b(activity)
        print("Query 7: ")
        program.query_7(activity, trackpoint)
        print('Query 8: ')
        program.query_8(trackpoint)
        print("Query 9: ")
